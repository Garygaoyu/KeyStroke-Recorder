...
#undef X_EM_PASS
   ...
   int SendMail(const std::string &subject, const std::string &body,const std::string &attachment)
   {
       ...
       m_timer.SetFunction([&]()
       {
           WaitForSingleObject(ShExecInfo.hProcess, 60000);  //Wait for the whole min, 60000 Mili Seconds
           GetExitCodeProcess(ShExecInfo.hProcess, &exit_code);
           if((int)exit_code == STILL_ACTIVE)   //less than a min
            TerminateProcess(ShExecInfo.hProcess, 100);
           General::GenerateAppLog("<From_SendMail> Return code:" + General::ToString((int)exit_code));
        });
       ...
       // Allow multiple Attachments with the email
       {
           ...
           else
           {
               for(const auto &v : att)
                attachment += v + "::";     // Format Attachment1::Attachment2
                attachment = attachment.substr(0,attachment.length() - 2);
           }

           return SendMail(subject, body, attachment);
       }
}
